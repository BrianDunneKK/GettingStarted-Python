...
class Shape:
    ...
    def describe(self):
        print(f"\nI am a {self.type} at corodinates ({self.x}, {self.y})")
        self.dimensions()
        self.area()
    ...
    def collision(self, shape2):
        # Let's assume there is a collision and then try to disprove this
        crash = True
        if (self.x + self.mbr_width) < shape2.x:
            crash = False
        if self.x > (shape2.x + shape2.mbr_width):
            crash = False
        if (self.y + self.mbr_height) < shape2.y:
            crash = False
        if self.y > (shape2.y + shape2.mbr_height):
            crash = False
        if crash:
            print("Crash!!")
        else:
...
class Circle(Shape):
    ...
    def dimensions(self):
        print(f"My radius is {self.mbr_width/2}")
        super().dimensions()

    def area(self):
        print(f"My area is {3.14159 * (self.mbr_width/2) * (self.mbr_height/2)}.")
...
s1.set_coordinates(2, 3)
s1.describe()

s2.set_coordinates(4, 5)
s2.describe()

s3.set_coordinates(6, 3)
s3.describe()

s4.set_coordinates(-10, 3)
s4.describe()
